package com.yunda.lib.view.progress;
...
public class CircularProgressBar extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            titleString = typedArray.getString(R.styleable.CircularProgressBar_cpb_title);
            textSize = typedArray.getDimensionPixelSize(R.styleable.CircularProgressBar_cpb_title_size, 12);
        } finally {
            typedArray.recycle();
        }
    ...
    private void drawText(Canvas canvas) {
        if (!TextUtils.isEmpty(titleString)) {
            vTextPaint.getTextBounds(titleString, 0, titleString.length(), fontRect);
            canvas.drawText(titleString, rectF.centerX(), rectF.centerY() + fontRect.height() / 2, vTextPaint);
        }
    }
    //endregion
